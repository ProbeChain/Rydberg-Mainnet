...
var (
	...
	// BlockRewardSmartLightPool is the per-block reward allocated to the SmartLight pool.
	// 0.2 PROBE per block → 6,000 PROBE per epoch (30,000 blocks).
...
// accumulateSmartLightRewards distributes the SmartLight reward pool
// proportionally to registered SmartLight nodes by their behavior score.
func accumulateSmartLightRewards(statedb *state.StateDB, snap *Snapshot) {
	if len(snap.SmartLights) == 0 {
		return
	}
	totalScore := snap.SmartLightTotalScore()
	if totalScore == 0 {
		return
	}
	pool := new(big.Int).Set(BlockRewardSmartLightPool)
	for addr, score := range snap.SmartLights {
		// reward = pool * nodeScore / totalScore
		reward := new(big.Int).Mul(pool, new(big.Int).SetUint64(score.Total))
