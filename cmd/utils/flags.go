...
var (
	...
	AgentModeFlag = cli.BoolFlag{
		Name:  "agentmode",
		Usage: "Enable PoB agent mode: headless operation, no mining, reduced memory (~30MB), max task throughput",
	}
...
func SetProbeConfig(ctx *cli.Context, stack *node.Node, cfg *probeconfig.Config) {
	...
	// Agent mode: enable PoB agent mode
	if ctx.GlobalBool(AgentModeFlag.Name) {
		cfg.AgentMode = true
	}
